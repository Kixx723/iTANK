...
import { Project, Workspace, NodeType, CountType } from "epanet-js";
...
const MapComponent: React.FC = () => {
  ...
  const addNode = (latlng: L.LatLng, type: string) => { 
   ...
   // Function to add a node to EPANET
   const addNodeToEpanet = (type: string) => {
    if (type === 'junction' || type === 'tank' || type === 'reservoir') {
        ...
        if (type === 'junction') {
            ...
            nodeIndex = model.addNode(`junction${nodeName.toString()}`, nodeType);
...
// };
  ...
  return (
    <>
    <SidePanel onSelectNodeType={setSelectedNodeType} onSelectLinkType={setSelectedLinkType}/>
    <MapContainer center={defaultPosition} zoom={zoomLevel} style={{ height: '100vh', width: '100%' }}>
      <TileLayer 
